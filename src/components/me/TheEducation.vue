...
<template>
    <!-- Education section. -->
    <div id="education" class="relative z-10 bg-purple-100 dark:bg-purple-900 polygon-section">
        <!-- Background colour transition. -->
        <div class="h-24 sm:h-28 lg:h-32 bg-gradient-to-b from-indigo-100 dark:from-indigo-900 to-purple-100 dark:to-purple-900" />
        ...
        <div class="flex flex-col sm:flex-row justify-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 lg:pt-8 pb-24 sm:pb-20 lg:pb-24">
            ...
            <div class="flex flex-col items-center justify-center mb-12 sm:mb-0 sm:mr-12 lg:mr-16">
                ...
                <h2 class="text-3xl sm:text-4xl tracking-tight font-extrabold text-gray-900 dark:text-gray-50">Education</h2>
            </div>
            <!-- Education feeds. -->
            <ul class="flow-root -mb-8 overflow-auto">
                <li v-for="(event, eventIndex) in timeline" :key="event.content">
                    <div class="relative pb-8">
                        ...
                        <span v-if="(eventIndex !== timeline.length - 1)" class="absolute top-6 left-6 -ml-px h-full w-0.5 bg-gray-300 dark:bg-gray-500" aria-hidden="true" />
                        <div class="relative flex space-x-3">
                            <template v-if="event.type === 'name'">
                                <img :src="event.imageUrl" class="relative h-12 w-12 rounded-full border-2 border-gray-300 dark:border-gray-600 flex items-center justify-center ring-8 ring-purple-100 dark:ring-purple-900" alt="Arvin: headshot" />
                                <div class="flex-1 pt-3.5 flex justify-between space-x-4">
                                    <p class="font-medium text-sm text-gray-900 dark:text-gray-50">{{ event.content }}</p>
                                    <p class="text-right text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{{ event.period }}</p>
                                </div>
                            </template>
                            <template v-else>
                                <div class="px-2">
                                    <span :class="[event.iconBackground, 'h-8 w-8 rounded-full flex items-center justify-center px-2 ring-8 ring-purple-100 dark:ring-purple-900']">
                                        <component :is="event.icon" class="h-5 w-5 text-gray-50" aria-hidden="true" />
                                    </span>
                                </div>
                                <a v-if="event.hasLink" :href="event.href" target="_blank" class="underline text-sm text-purple-400 hover:text-purple-500 dark:hover:text-purple-300 motion-safe:transition-colors motion-safe:duration-300 pt-1.5">
                                    {{ event.content }}
                                </a>
                                <p v-else class="text-sm text-gray-500 dark:text-gray-300 pt-1.5">{{ event.content }}</p>
                            </template>
                        </div>
                    </div>
                </li>
...
export default {
    ...
    setup() {
        const eventIcons = {
            check: { icon: CheckIcon, background: "bg-green-500 dark:bg-green-700" },
        ...
        const timeline = [
            ...
            {
                content: "Dalian Jiahui Sunshine Primary School",
                imageUrl: ArvinHeadshotPrimary2,
                period: "2009 - 2010",
                type: "name"
            },
            {
                content: "Dalian Jiahui Middle School",
                imageUrl: ArvinHeadshotMiddle,
                period: "2010 - 2013",
                type: "name"
            },
            {
                content: "Dalian Yuming Senior High School",
                imageUrl: ArvinHeadshotSenior,
                period: "2013 - 2016",
                type: "name"
            },
            {
                content: "Beihang University",
            ...
            {
                content: "Several scholarships awarding study on language and professional skills",
                icon: eventIcons.happy.icon,
                iconBackground: eventIcons.happy.background,
                type: "highlight"
            },
            {
                content: "SQA Ambassadors China 2019",
                icon: eventIcons.flag.icon,
                iconBackground: eventIcons.flag.background,
                type: "highlight"
            },
            {
                content: "University of Lincoln",
            ...
            {
                content: "The University of Lincoln 50% Global Scholarship 2019",
                icon: eventIcons.happy.icon,
                iconBackground: eventIcons.happy.background,
                type: "highlight"
            },
            {
                content: "MTA: Software Development Fundamentals - Certified 2019",
                hasLink: true,
            ...
            {
                content: "MS in Computing Science, working hard✍️",
